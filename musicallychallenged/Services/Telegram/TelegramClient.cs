...
namespace musicallychallenged.Services.Telegram
{
    ...
    public class TelegramClient : ITelegramClient
    {
        ...
        public TelegramClient(BotConfiguration configuration)
        {
        ...
        {
            await ExecuteThrottled(() => _client.DeleteMessageAsync(chatId, messageId, cancellationToken).ContinueWith(s=>0), cancellationToken);
        }
        ...
        {
            await ExecuteThrottled(() =>_client.AnswerCallbackQueryAsync(callbackQueryId, text, showAlert, url, cacheTime, cancellationToken).ContinueWith(s=>0), cancellationToken);
        }
        ...
        public async Task<Message> ForwardMessageAsync(ChatId chatId, ChatId fromChatId, int messageId,
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return await ExecuteThrottled(() =>_client.ForwardMessageAsync(chatId, fromChatId, messageId, disableNotification, cancellationToken), cancellationToken);
        }
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            try
            {
                ...
                return await _client.EditMessageReplyMarkupAsync(chatId, messageId, replyMarkup, cancellationToken);
            }
            catch (Exception e)
            {
        ...
        public async Task PinChatMessageAsync(ChatId chatId, int messageId, bool disableNotification = false, CancellationToken cancellationToken = default(CancellationToken))
        {
            try
            {
                //Guard PinChatMessageAsync: it's ok to silently eat exceptions on markup edit failure
                await ExecuteThrottled(() => _client.PinChatMessageAsync(chatId, messageId, disableNotification, cancellationToken).ContinueWith(t=>0), cancellationToken);                                
            }
            catch (Exception e)
            {
        ...
        private DateTime? _lastSendDateTime = null;
