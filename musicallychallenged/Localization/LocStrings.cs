...
namespace musicallychallenged.Localization
{
    ...
    public class LocStrings
    {
        ...
        public string ThankYouForVote { get; set; } = $"Спасибо, ваш голос ({LocTokens.VoteCount}) учтен ";
        public string VoteRemoved { get; set; } = $"Ваш голос ({LocTokens.VoteCount}) отменен";
...
Написать работу и отправить ее боту @nsctheorbot с помощью команды /<code>submit</code> (команда отсылается в личке боту) нужно до <b>{LocTokens.Deadline}</b>
Текстовое описание к работе (по желанию) можно отправить командой /<code>describe</code>

Желательно, чтобы у вас был настроен юзернейм (как его создать можете прочитать здесь (https://telegra.ph/How-to-set-nickname-in-Telegram-07-08)).
...
Если победивших будет 2 или больше, тот, чье задание будет взято для следующего челленджа определит рандомайзер. Если победитель не объявится, будет использовано случайное задание из пула с идеями (https://t.me/pesnopisetsidei)
...
Если у вас будут какие-то идеи для новый челленджей, пишите в чат. Администрация добавит их в этот канал (https://t.me/pesnopisetsidei)
